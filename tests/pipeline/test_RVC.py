...
# run the RVC module using this config_RVC_norun (which should do nothing)
    ...
    def pipeline_run(self, demo_dir):
        ...
        print("HERE")
        pipeline_run = run(["snakemake", "rnaVariantCalling", f"-c{CORES}"], demo_dir)
    ...
    def test_variants_joint(self, demo_dir):
        vcf_file0 = "Output/processed_data/rnaVariantCalling/out/all_samples_haplocaller/batch_0_all_samples.genotyped.vcf.gz"
        vcf_file1 = "Output/processed_data/rnaVariantCalling/out/all_samples_haplocaller/batch_1_all_samples.genotyped.vcf.gz"
        r_cmd = """ 
                library(data.table)
                vcf0  <- fread("{}")
    ...
    def test_variants_single_line_multi(self, demo_dir):
        vcf_file0 = "Output/processed_data/rnaVariantCalling/out/all_samples_haplocaller/batch_0_all_samples.genotyped.filtered_clean.vcf.gz"
        vcf_file1 = "Output/processed_data/rnaVariantCalling/out/all_samples_haplocaller/batch_1_all_samples.genotyped.filtered_clean.vcf.gz"
        r_cmd = """ 
                library(data.table)
                vcf0  <- fread("{}")
                num_variants0 <- nrow(vcf0)
