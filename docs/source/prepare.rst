...
++++++++++++++++++++++++++++++++++
RNA Variant Calling may be useful for researchers who don't have access to variant calls from genomic data. While variant calling from WES and WGS technologies may be more traditional, we have found that variant calling from RNA-Seq data can provide additional evidence for the underlying causes of aberrant expression or splicing.
...
Box 3 of the `DROP manuscript <https://www.nature.com/articles/s41596-020-00462-5>`_.
...
To run the MAE module, the columns ``DNA_ID`` and ``DNA_VCF_FILE`` are needed. To run the rnaVariantCalling module, the column ``RNA_VARIANT_GROUP`` is needed.
