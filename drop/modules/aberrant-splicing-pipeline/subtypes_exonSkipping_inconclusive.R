...
checkExonSkipping <- function(junctions_dt, txdb){
  ...
  exonSkip <- which(junctions_dt[psi_positions]$aberrantSpliceType %in% c("multipleExonSkipping", "exonSkipping"))
  ...
  newSkip_results <- sapply(exonSkip, function(i){
    ...
    for(to in start_genes){
      ## end is in same gene
      if(distance(test_end, refseq.genes[to]) == 0) return("multipleExonSkipping")
    }
