...
groups <- snakemake@params$datasets
anno_version <- snakemake@params$annotations
html_file_dir <- gsub(snakemake@config$htmlOutputPath, '.', 
                      dirname(dirname(snakemake@params$html)))
summaries_titles <- sapply(anno_version, function(v) {
        paste0('[', groups ,'](', html_file_dir, '/',
               v, '/Summary_', groups, '.html){target="_blank"}', 
              collapse = ' ')
    }
)

#' ## Count Summaries
#' `r paste0('Gene annotation version ', names(summaries_titles),
#'           ': ', summaries_titles, collapse = '\n')`
